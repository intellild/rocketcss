...
impl<'i> Parse<'i> for Mask<'i> {
    fn parse(input: &mut Compiler<'i>) -> Result<Self, ParseError<'i, ParserError<'i>>> {
        ...
        loop {
            ...
            {
                image = Some(allocator.boxed(value));
                continue;
            }
            ...
            {
                position = Some(allocator.boxed(Position { x, y }));
                size = input
                    .try_parse(|input| {
                        input.expect_delim('/')?;
        ...
        Ok(Self {
            image: image.unwrap_or_else(|| allocator.boxed(Image::None)),
            position: position.unwrap_or_else(|| allocator.boxed(default_mask_position(allocator))),
            size: size.unwrap_or_else(|| allocator.boxed(default_mask_size(allocator))),
...
fn default_mask_position<'i>(allocator: &'i Allocator) -> Position<'i> {
    Position {
        x: zero_mask_position_component(allocator),
        y: zero_mask_position_component(allocator),
    }
}

fn zero_mask_position_component<'i, S>(
    allocator: &'i Allocator,
) -> Box<'i, PositionComponent<'i, S>> {
    allocator.boxed(PositionComponent::Length(
        allocator.boxed(DimensionPercentage::Percentage(0.0)),
    ))
}

fn default_mask_size<'i>(allocator: &'i Allocator) -> BackgroundSize<'i> {
