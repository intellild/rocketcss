...
use crate::AstType;
use rocketcss_ast::*;
pub fn walk_selector_component<'a, VisitorT>(
    ...
    VisitorT: ?Sized + VisitMut<'a>,
{
    ...
    match node {
        ...
        SelectorComponent::AttributeOther(field_0) => {
            visitor.visit_attr_selector((field_0).as_mut());
        }
        ...
        SelectorComponent::NthOf { data, selectors } => {
            ...
            for value_2 in (selectors).iter_mut() {
                visitor.visit_selector(value_2);
            }
        }
        SelectorComponent::PseudoClass(field_0) => {
            visitor.visit_pseudo_class((field_0).as_mut());
        }
        SelectorComponent::Slotted(field_0) => {
            visitor.visit_selector((field_0).as_mut());
        }
        ...
        SelectorComponent::Host(field_0) => {
            if let Some(value_6) = (field_0).as_mut() {
                visitor.visit_selector((value_6).as_mut());
            }
        }
        SelectorComponent::Where(field_0) => {
            for value_8 in (field_0).iter_mut() {
        ...
        SelectorComponent::Any {
            ...
            for value_10 in (selectors).iter_mut() {
        ...
        SelectorComponent::PseudoElement(field_0) => {
            visitor.visit_pseudo_element((field_0).as_mut());
        }
        SelectorComponent::Nesting => {}
    }
    visitor.leave_node(AstType::SelectorComponent);
}
pub fn walk_combinator<'a, VisitorT>(visitor: &mut VisitorT, node: &mut Combinator)
where
    VisitorT: ?Sized + VisitMut<'a>,
{
...
pub fn walk_attr_selector<'a, VisitorT>(visitor: &mut VisitorT, node: &mut AttrSelector<'a>)
where
    VisitorT: ?Sized + VisitMut<'a>,
{
    visitor.enter_node(AstType::AttrSelector);
    if let Some(value_0) = (&mut node.namespace).as_mut() {
        visitor.visit_namespace_constraint(value_0);
    }
    visitor.visit_atom(&mut node.local_name);
    visitor.visit_atom(&mut node.local_name_lower);
    visitor.visit_attr_operation(&mut node.operation);
...
pub fn walk_attr_operation<'a, VisitorT>(visitor: &mut VisitorT, node: &mut AttrOperation<'a>)
where
    VisitorT: ?Sized + VisitMut<'a>,
{
...
pub fn walk_nth_type<'a, VisitorT>(visitor: &mut VisitorT, node: &mut NthType)
where
    VisitorT: ?Sized + VisitMut<'a>,
{
...
pub fn walk_nth_selector_data<'a, VisitorT>(visitor: &mut VisitorT, node: &mut NthSelectorData)
where
    VisitorT: ?Sized + VisitMut<'a>,
{
    visitor.enter_node(AstType::NthSelectorData);
    visitor.visit_nth_type(&mut node.kind);
    visitor.leave_node(AstType::NthSelectorData);
}
pub fn walk_direction<'a, VisitorT>(visitor: &mut VisitorT, node: &mut Direction)
where
    VisitorT: ?Sized + VisitMut<'a>,
{
...
pub fn walk_pseudo_class<'a, VisitorT>(visitor: &mut VisitorT, node: &mut PseudoClass<'a>)
where
    VisitorT: ?Sized + VisitMut<'a>,
{
    ...
    match node {
        PseudoClass::Lang { languages } => {
            for value_0 in (languages).iter_mut() {
        ...
        PseudoClass::Local { selector } => {
            visitor.visit_selector((selector).as_mut());
        }
        ...
        PseudoClass::CustomFunction { name, arguments } => {
            visitor.visit_atom(name);
            for value_4 in (arguments).iter_mut() {
...
pub fn walk_pseudo_element<'a, VisitorT>(visitor: &mut VisitorT, node: &mut PseudoElement<'a>)
where
    VisitorT: ?Sized + VisitMut<'a>,
{
    ...
    match node {
        ...
        PseudoElement::ViewTransitionGroup { part } => {
            visitor.visit_view_transition_part_selector((part).as_mut());
        }
        ...
        PseudoElement::CustomFunction { name, arguments } => {
            visitor.visit_atom(name);
            for value_6 in (arguments).iter_mut() {
...
pub fn walk_selector_list<'a, VisitorT>(visitor: &mut VisitorT, node: &mut SelectorList<'a>)
where
    VisitorT: ?Sized + VisitMut<'a>,
{
...
pub fn walk_selector<'a, VisitorT>(visitor: &mut VisitorT, node: &mut Selector<'a>)
where
    VisitorT: ?Sized + VisitMut<'a>,
{
