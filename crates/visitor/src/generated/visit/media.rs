    ...
    VisitorT: ?Sized + Visit<'a>,
{
    ...
    match node {
        ...
        MediaCondition::Not(field_0) => {
            visitor.visit_media_condition((field_0).as_ref());
        }
    ...
    FeatureId: VisitNode<'a, VisitorT>,
{
    ...
    match node {
        QueryFeature::Plain { name, value } => {
            visitor.visit_media_feature_name((name).as_ref());
            visitor.visit_media_feature_value((value).as_ref());
        }
        ...
        QueryFeature::Interval {
            ...
            visitor.visit_media_feature_value((end).as_ref());
            visitor.visit_media_feature_comparison(end_operator);
            visitor.visit_media_feature_name((name).as_ref());
            visitor.visit_media_feature_value((start).as_ref());
...
pub fn walk_media_feature_id<'a, VisitorT>(visitor: &mut VisitorT, node: &MediaFeatureId)
where
    VisitorT: ?Sized + Visit<'a>,
{
...
pub fn walk_media_feature_value<'a, VisitorT>(visitor: &mut VisitorT, node: &MediaFeatureValue<'a>)
where
    VisitorT: ?Sized + Visit<'a>,
{
    ...
    match node {
        ...
        MediaFeatureValue::Ratio(field_0) => {
            visitor.visit_ratio((field_0).as_ref());
        }
...
pub fn walk_operator<'a, VisitorT>(visitor: &mut VisitorT, node: &Operator)
where
    VisitorT: ?Sized + Visit<'a>,
{
...
pub fn walk_media_type<'a, VisitorT>(visitor: &mut VisitorT, node: &MediaType<'a>)
where
    VisitorT: ?Sized + Visit<'a>,
{
...
pub fn walk_qualifier<'a, VisitorT>(visitor: &mut VisitorT, node: &Qualifier)
where
    VisitorT: ?Sized + Visit<'a>,
{
...
pub fn walk_supports_condition<'a, VisitorT>(visitor: &mut VisitorT, node: &SupportsCondition<'a>)
where
    VisitorT: ?Sized + Visit<'a>,
{
    ...
    match node {
        SupportsCondition::Not(field_0) => {
            visitor.visit_supports_condition((field_0).as_ref());
        }
        ...
        SupportsCondition::Declaration { property_id, value } => {
            visitor.visit_property_id((property_id).as_ref());
...
pub fn walk_media_feature<'a, VisitorT>(visitor: &mut VisitorT, node: &MediaFeature<'a>)
where
    VisitorT: ?Sized + Visit<'a>,
{
