...
impl ToCss for Function<'_> {
    fn to_css<PrinterT: PrinterTrait>(&self, dest: &mut PrinterT) -> fmt::Result {
        ...
        if matches_ignore_ascii_case(self.name, &["var", "env", "constant"])
...
fn write_minified_rgba<PrinterT: PrinterTrait>(
    ...
    if !use_hex {
        dest.write_str("rgba(")?;
        serialize_number(red as f32, dest)?;
        dest.write_char(',')?;
        serialize_number(green as f32, dest)?;
        dest.write_char(',')?;
        serialize_number(blue as f32, dest)?;
        dest.write_char(',')?;
        serialize_number(alpha, dest)?;
        return dest.write_char(')');
    }
    ...
    if values.iter().all(|value| value >> 4 == value & 15) {
        for value in values {
            dest.write_char(HEX[(value & 15) as usize] as char)?;
        }
    } else {
        for value in values {
            dest.write_char(HEX[(value >> 4) as usize] as char)?;
            dest.write_char(HEX[(value & 15) as usize] as char)?;
        }
    }
    Ok(())
...
fn write_minified_rgb<PrinterT: PrinterTrait>(
    ...
    if red >> 4 == red & 15 && green >> 4 == green & 15 && blue >> 4 == blue & 15 {
        dest.write_char(HEX[(red & 15) as usize] as char)?;
        dest.write_char(HEX[(green & 15) as usize] as char)?;
        dest.write_char(HEX[(blue & 15) as usize] as char)
    } else {
