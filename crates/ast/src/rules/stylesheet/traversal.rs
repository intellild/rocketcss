use super::*;

/// Storage-neutral callbacks over the semantic Radix AST.
///
/// Traversal owns ordering. Callbacks receive typed IDs and may resolve any
/// additional immutable data through `stylesheet` without depending on store
/// layout or retaining references across a later mutation.
pub trait StyleSheetVisitor<'ast> {
    fn visit_rule(
        &mut self,
        _id: CssRuleId<'ast>,
        _rule: &RuleRecord<CssRule<'ast>>,
        _stylesheet: &StyleSheet<'ast>,
    ) {
    }

    fn visit_declaration_block(
        &mut self,
        _id: CssDeclarationBlockId<'ast>,
        _block: &DeclarationBlock<CssRule<'ast>>,
        _stylesheet: &StyleSheet<'ast>,
    ) {
    }

    fn visit_declaration(
        &mut self,
        _block: CssDeclarationBlockId<'ast>,
        _id: DeclarationId,
        _declaration: &DeclarationRecord<CssDeclaration<'ast>>,
        _stylesheet: &StyleSheet<'ast>,
    ) {
    }

    fn visit_descriptor(
        &mut self,
        _block: CssDeclarationBlockId<'ast>,
        _id: DeclarationId,
        _descriptor: &DeclarationRecord<CssDeclaration<'ast>>,
        _stylesheet: &StyleSheet<'ast>,
    ) {
    }
}

/// Mutable Radix traversal callbacks.
///
/// The context deliberately exposes semantic replacement transactions rather
/// than `&mut` store entries. A callback therefore cannot retain a reference
/// that an insertion or interner update would invalidate.
pub trait StyleSheetVisitorMut<'ast> {
    fn visit_selector_value(
        &mut self,
        _id: SelectorValueId,
        _selectors: &mut crate::SelectorList<'ast>,
    ) {
    }

    fn visit_rule(&mut self, _id: CssRuleId<'ast>, _cx: &mut StyleSheetVisitMutContext<'_, 'ast>) {}

    fn visit_declaration_block(
        &mut self,
        _id: CssDeclarationBlockId<'ast>,
        _cx: &mut StyleSheetVisitMutContext<'_, 'ast>,
    ) {
    }

    fn visit_declaration(
        &mut self,
        _block: CssDeclarationBlockId<'ast>,
        _id: DeclarationId,
        _cx: &mut StyleSheetVisitMutContext<'_, 'ast>,
    ) {
    }

    fn visit_descriptor(
        &mut self,
        _block: CssDeclarationBlockId<'ast>,
        _id: DeclarationId,
        _cx: &mut StyleSheetVisitMutContext<'_, 'ast>,
    ) {
    }
}

/// Mutation access available during [`StyleSheetVisitorMut`] traversal.
pub struct StyleSheetVisitMutContext<'comp, 'ast> {
    stylesheet: &'comp mut StyleSheet<'ast>,
}

impl<'comp, 'ast> StyleSheetVisitMutContext<'comp, 'ast> {
    #[inline]
    pub fn stylesheet(&self) -> &StyleSheet<'ast> {
        self.stylesheet
    }

    #[inline]
    pub fn replace_rule_selector_value(
        &mut self,
        rule: CssRuleId<'ast>,
        selector: SelectorValueId,
    ) -> Result<bool, StyleSheetMutationError<'ast>> {
        self.stylesheet.replace_rule_selector_value(rule, selector)
    }

    #[inline]
    pub fn replace_property_declaration(
        &mut self,
        block: CssDeclarationBlockId<'ast>,
        declaration: DeclarationId,
        replacement: crate::Declaration<'ast>,
    ) -> Result<crate::Declaration<'ast>, StyleSheetMutationError<'ast>> {
        if !matches!(
            self.stylesheet
                .declaration(declaration)
                .map(DeclarationRecord::payload),
            Some(CssDeclaration::Property(_))
        ) {
            return Err(StyleSheetMutationError::UnknownDeclaration(declaration));
        }
        let previous = self.stylesheet.replace_declaration(
            block,
            declaration,
            CssDeclaration::Property(replacement),
        )?;
        let CssDeclaration::Property(previous) = previous else {
            unreachable!("the declaration kind was validated before replacement")
        };
        Ok(previous)
    }

    #[inline]
    pub fn intern_selector_value(
        &mut self,
        selectors: crate::SelectorList<'ast>,
        kind: SelectorFrameKind,
        vendor_prefix: crate::VendorPrefix,
    ) -> Result<SelectorValueId, StyleSheetMutationError<'ast>> {
        self.stylesheet
            .intern_selector_value(selectors, kind, vendor_prefix)
    }
}

impl<'ast> StyleSheet<'ast> {
    /// Visits live rules in lexical preorder, followed by each rule's owned
    /// declaration block and ordered declaration occurrences.
    pub fn visit_stylesheet<V: ?Sized + StyleSheetVisitor<'ast>>(
        &self,
        visitor: &mut V,
    ) -> Result<(), StyleSheetMutationError<'ast>> {
        for (rule_id, rule) in self.rules_in_source_order() {
            if !rule.is_live() {
                continue;
            }
            visitor.visit_rule(rule_id, rule, self);
            let Some(block_id) = rule.declaration_block() else {
                continue;
            };
            let block = self
                .declaration_block(block_id)
                .ok_or(StyleSheetMutationError::UnknownDeclarationBlock(block_id))?;
            if !block.is_live() {
                return Err(StyleSheetMutationError::InvalidRuleTopology(rule_id));
            }
            let property_block = rule.payload().owns_property_declarations();
            if property_block {
                visitor.visit_declaration_block(block_id, block, self);
            }
            for (declaration_id, declaration) in self.declaration_occurrences_in_block(block_id)? {
                if property_block {
                    visitor.visit_declaration(block_id, declaration_id, declaration, self);
                } else {
                    visitor.visit_descriptor(block_id, declaration_id, declaration, self);
                }
            }
        }
        Ok(())
    }

    /// Mutably visits the same semantic sequence as [`Self::visit_stylesheet`].
    ///
    /// Only selector and declaration replacement transactions are exposed to
    /// callbacks. Structural mutations stay scheduler-owned, so the captured
    /// source successor and declaration identities remain valid for the callback.
    pub fn visit_stylesheet_mut<V: ?Sized + StyleSheetVisitorMut<'ast>>(
        &mut self,
        visitor: &mut V,
    ) -> Result<(), StyleSheetMutationError<'ast>> {
        self.transform_selector_values(|id, selectors| {
            visitor.visit_selector_value(id, selectors);
        });
        let mut current = self.first_rule_in_source;
        while let Some(rule_id) = current {
            let rule = self
                .rule(rule_id)
                .ok_or(StyleSheetMutationError::UnknownRule(rule_id))?;
            current = rule.next_in_source();
            if !rule.is_live() {
                continue;
            }

            visitor.visit_rule(rule_id, &mut StyleSheetVisitMutContext { stylesheet: self });
            let rule = self
                .rule(rule_id)
                .ok_or(StyleSheetMutationError::UnknownRule(rule_id))?;
            let property_block = rule.payload().owns_property_declarations();
            let Some(block_id) = rule.declaration_block() else {
                continue;
            };
            let block = self
                .declaration_block(block_id)
                .ok_or(StyleSheetMutationError::UnknownDeclarationBlock(block_id))?;
            if !block.is_live() {
                return Err(StyleSheetMutationError::InvalidRuleTopology(rule_id));
            }
            if property_block {
                visitor.visit_declaration_block(
                    block_id,
                    &mut StyleSheetVisitMutContext { stylesheet: self },
                );
            }
            let declaration_count = self.declaration_ids_in_block(block_id)?.len();
            for index in 0..declaration_count {
                let declaration_id = self.declaration_id_at_in_block(block_id, index)?;
                if property_block {
                    visitor.visit_declaration(
                        block_id,
                        declaration_id,
                        &mut StyleSheetVisitMutContext { stylesheet: self },
                    );
                } else {
                    visitor.visit_descriptor(
                        block_id,
                        declaration_id,
                        &mut StyleSheetVisitMutContext { stylesheet: self },
                    );
                }
            }
        }
        Ok(())
    }
}
