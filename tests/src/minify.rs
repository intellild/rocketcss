...
use rocketcss_ast::{CssRulePayload, Span};
...
#[test]
fn minifies_enabled_s5_upstream_fixtures() {
    let mut fixture_count = 0;
    for input in fixture_paths("minify") {
        let path = fixture_path_key(&input);
        if is_enabled_s5_upstream_fixture(&path) {
            assert!(!still_requires_unsupported_transform(&input));
            assert_minifies_static_fixture(&input);
            fixture_count += 1;
        }
    }
...
fn synthesized_cross_rule_fixture_preserves_combined_source_span() {
    ...
    allocator.with_ghost(|mut token| {
        ...
        let CssRulePayload::Style(_) = rules[0].1.payload() else {
            panic!("expected one synthesized style rule");
        };
        assert_eq!(
            stylesheet.rule_span(rules[0].0),
...
fn still_requires_unsupported_transform(input: &Path) -> bool {
    ...
    let unsupported_cases = [
        ...
        "/cssnano/normalize-positions/pair/",
        "/cssnano/normalize-positions/single/",
...
fn is_enabled_s5_upstream_fixture(path: &str) -> bool {
    [
